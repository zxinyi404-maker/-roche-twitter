# Twitter 插件更新日志

## v1.5.3 (2026-07-29)

### ✨ 新增功能

**关注和粉丝列表**
- ✅ 侧边栏「正在关注」和「关注者」现在可以点击
- ✅ 新增关注列表页面（显示你关注的所有人）
- ✅ 新增粉丝列表页面（显示关注你的所有人）
- ✅ 每个用户卡片显示头像、名字、用户名、简介
- ✅ 直接在列表中关注/取消关注用户
- ✅ 点击用户卡片查看个人资料

### 🎨 用户体验优化

**列表页面**
- 空状态提示（还没有关注任何人 / 还没有粉丝）
- 自己不显示关注按钮
- 关注按钮样式清晰：未关注=黑底白字，已关注=白底黑字
- 操作后自动刷新列表
- 返回按钮回到主页

**样式优化**
- 移除首页空状态的 emoji

### 🐛 Bug 修复

**侧边栏功能**
- 修复「设置和隐私」功能逻辑，直接进入切换账号页面
- 明确区分：侧边栏设置=切换人设，右上角设置=插件功能设置

### 技术改进

**视图管理**
- switchView 新增 followingList 和 followersList 视图
- 新增 showFollowingList 和 showFollowersList 函数
- 完善的事件绑定和清理机制

---

## v1.5.2 (2026-07-29)

### 🎯 核心功能升级

**真正的 Persona 切换**
- ✅ 集成 Roche API `setActiveUserPersona()` 
- ✅ 切换账号时同步更新系统级 Persona
- ✅ 不仅切换插件内用户，还切换整个 Roche 的活跃人设

**账号管理优化**
- ✅ "刷新账号列表"功能（替代"添加已有账号"）
- ✅ 自动检测并导入 Roche 系统中的所有 Persona
- ✅ 错误处理优化：切换失败时显示详细错误信息

### 具体变更

#### 1. `switchPersona()` 函数重写
```javascript
// 新增功能：
- 调用 roche.persona.setActiveUserPersona(personaId)
- 真正切换系统级 Persona（不仅是插件内部）
- 完整的错误捕获和用户提示
- 只有有 conversationId 的 Persona 才调用 API
```

#### 2. "刷新账号列表" 功能
```javascript
// 点击按钮后：
- 调用 roche.persona.getUserPersonas()
- 自动发现新增的 Persona
- 更新 twitterData.users
- 重新渲染切换账号页面
```

#### 3. 权限更新
新增 `persona:write` 权限到 manifest.json：
- 允许插件修改活跃 Persona
- 必需用于 `setActiveUserPersona()` API

### 技术实现

**API 集成：**
- `roche.persona.setActiveUserPersona(personaId)` - 切换活跃 Persona
- `roche.persona.getUserPersonas()` - 重新获取所有 Persona
- 错误处理：try-catch 包裹所有异步操作

**文件修改：**
- `plugin.js` - switchPersona() 函数重写
- `plugin.js` - 添加账号按钮改为刷新功能
- `manifest.json` - 版本 1.5.1 → 1.5.2
- `manifest.json` - 新增 persona:write 权限

### 用户体验提升

✨ **真正的多人设系统**
- 切换账号后，整个 Roche 的人设都会改变
- 不仅影响 Twitter 插件，还影响所有对话
- 真正实现"角色扮演"切换

✨ **更智能的账号管理**
- 不需要手动"添加"账号
- 自动检测 Roche 中的所有 Persona
- 一键刷新即可同步

### 向后兼容

✅ 完全兼容 v1.5.1 的所有功能
✅ 不影响现有数据结构
✅ 平滑升级，无需清除数据

---

## v1.4.2 (2026-07-29)

### 功能优化

**账号管理重构**
- ✅ 将"切换账号"功能从侧边栏移动到设置页面
- ✅ 在设置页添加"账号管理"分组
- ✅ 新增独立的"切换账号"页面
- ✅ 优化账号切换交互体验

### 具体变更

#### 1. 侧边栏简化
- 移除侧边栏底部的"切换账号"区域
- 侧边栏现在只显示当前用户信息和菜单项

#### 2. 设置页新增功能
设置页现在包含以下分组：

**【账号管理】**
- 切换账号 → 进入账号切换页面
- 添加已有账号 → 添加新账号（开发中）

**【账号设置】**
- 编辑个人资料
- 隐私和安全
- 通知

**【记忆设置】**
- 启用推文记忆
- 自动总结对话
- 记忆保存到：当前会话
- 清除所有记忆

**【关于】**
- 版本：v1.4.2
- 关于 Twitter 插件

#### 3. 新增"切换账号"页面
独立页面显示：

**当前账号区域**
- 显示当前登录账号
- 头像、名字、用户名
- ✓ 当前账号标记

**其他账号列表**
- 显示所有可切换的 Persona 账号
- 点击即可切换
- 自动返回主页

**底部操作**
- + 添加已有账号按钮

#### 4. 交互优化
- 点击某个账号 → 切换 currentUser
- 显示 Toast 提示："已切换到 XX"
- 自动返回主页并刷新内容
- 平滑的页面切换动画

### 技术实现

**代码变更：**
- 移除 `sidebar-personas` 区域的 HTML 和渲染逻辑
- 新增 `switch-account-view` 页面视图
- 添加 `showSwitchAccount()` 函数
- 更新 `switchView()` 支持新视图
- 更新 `showSettings()` 绑定新菜单项
- 简化 `openSidebar()` 移除账号列表渲染

**文件修改：**
- `plugin.js` - 主要功能实现
- 版本号：1.4.1 → 1.4.2

### 用户体验提升

✨ **更符合主流社交应用习惯**
- 账号切换功能统一放在设置中
- 独立页面展示，信息更清晰
- 操作流程更直观

✨ **视觉优化**
- 当前账号有明显的 ✓ 标记
- 其他账号列表清晰分组
- 保持 Twitter 黑白极简风格

### 向后兼容

✅ 完全兼容现有数据结构
✅ 不影响现有推文、关注等功能
✅ 保持所有记忆系统功能正常

---

## 之前版本

### v1.4.1
- 集成记忆系统
- 新闻推文功能
- 多 Persona 支持

### v1.4.0
- 初始版本发布
- 完整 Twitter 克隆功能
- 黑白极简 UI 设计
